...
namespace Azure.Storage
{
	public class BlobStorageHelper : StorageHelper
	{
		...
		*/
		...
		public byte[] GetBlob(string container, string name, out Dictionary<string, string> metaData)
		{
		...
		{
			string uri = string.Format("{0}/{1}", container, name);
			var response = ExecuteRestRequestWithFailover(HtmlVerb.HEAD, uri);
			if (!response.IsSuccessStatusCode)
			{
		...
		public bool PutBlob(string container, string name, byte[] content, Dictionary<string, string> metaData = null)
		{
